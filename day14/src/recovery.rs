...
impl RecoveryManager {
    ...
    pub fn recover(
        ...
        let checkpoint_lsn = checkpoint::read_checkpoint_lsn(data_dir)?;
        println!("[Recovery] Checkpoint LSN: {:?}", checkpoint_lsn);

        // Read WAL records from checkpoint position (or beginning if no checkpoint)
        let wal_records = read_wal_records_from(wal_dir, checkpoint_lsn)?;
        ...
        let analysis = Self::analyze(&wal_records, checkpoint_lsn);
        println!(
        ...
        txn_manager.set_next_txn_id(analysis.max_txn_id + 1);
