...
//   Header (12 bytes):
//     - page_id: u32 (4 bytes)
//     - next_page_id: u32 (4 bytes) - u32::MAX means no next page
//     - tuple_count: u16 (2 bytes)
//     - free_space_offset: u16 (2 bytes) - points to end of free space
//   Slot array (grows forward from offset 12):
...
impl HeapPage {
    ...
    pub fn set_next_page_id(&mut self, next_page_id: u32) {
        self.data[4..8].copy_from_slice(&next_page_id.to_ne_bytes());
    }

    pub fn tuple_count(&self) -> u16 {
        u16::from_ne_bytes(self.data[8..10].try_into().unwrap())
    }

    fn set_tuple_count(&mut self, count: u16) {
        self.data[8..10].copy_from_slice(&count.to_ne_bytes());
    }

    pub fn free_space_offset(&self) -> u16 {
        u16::from_ne_bytes(self.data[10..12].try_into().unwrap())
    }

    fn set_free_space_offset(&mut self, offset: u16) {
        self.data[10..12].copy_from_slice(&offset.to_ne_bytes());
    }
