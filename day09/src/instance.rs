...
impl Instance {
        ...
        loop {
            match conn.read_message()? {
                Some(FrontendMessage::Query(sql)) => {
                    ...
                    match Self::execute_sql(&sql, &catalog, &bpm, &mut txn) {
    ...
    fn execute_sql(
        ...
        // Pass transaction to engine for undo log recording
        ...
        match &analyzed {
            AnalyzedStatement::Select(select_stmt) => {
                ...
                let mut engine =
                    ExecutionEngine::new(Arc::clone(bpm), catalog, &analyzed, txn_opt)?;
