        ...
        for stream in listener.incoming() {
            match stream {
                Ok(stream) => {
                    ...
                    thread::spawn(move || {
                        if let Err(e) = Self::handle_client(conn, catalog, bpm) {
        ...
        loop {
            match conn.read_message()? {
                Some(FrontendMessage::Query(sql)) => {
                    ...
                    match Self::execute_sql(&sql, &catalog, &bpm) {
                        Ok(result) => {
    ...
    fn execute_sql(
        ...
        match &analyzed {
            AnalyzedStatement::Select(select_stmt) => {
                ...
                let mut engine =
                    ExecutionEngine::new(Arc::clone(bpm), catalog, &analyzed)?;
            ...
            AnalyzedStatement::Delete(_) => {
                let mut engine =
                    ExecutionEngine::new(Arc::clone(bpm), catalog, &analyzed)?;
                let results = engine.execute()?;
                let count = if let Some(tuple) = results.first() {
                    if let Value::Int(n) = tuple.values[0] {
                        n
                    } else {
                        0
                    }
