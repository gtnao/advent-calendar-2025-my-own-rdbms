...
impl<'a> SeqScanExecutor<'a> {
    pub fn new(bpm: &'a mut BufferPoolManager, catalog: &'a Catalog, table_id: usize) -> Self {
        SeqScanExecutor {
            bpm,
            catalog,
            table_id,
...
impl<'a> InsertExecutor<'a> {
    pub fn new(bpm: &'a mut BufferPoolManager, stmt: &AnalyzedInsertStatement) -> Result<Self> {
...
impl Executor for InsertExecutor<'_> {
    ...
    fn next(&mut self) -> Result<Option<Tuple>> {
        ...
        if page_count > 0 {
            ...
            if let Ok(_slot_id) = page.insert(&tuple_data) {
                self.bpm.unpin_page(last_page_id, true)?;
                return Ok(Some(Tuple::new(vec![Value::Int(1)]))); // 1 row inserted
            }
