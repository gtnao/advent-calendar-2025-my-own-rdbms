...
use crate::page::btree_meta::MetaNode;
...
impl BTree {
    ...
    /// Create an empty B-Tree with a meta page and a single leaf node as root
    pub fn create_empty(&mut self) -> Result<()> {
        ...
        {
            let mut page_guard = meta_page.write().unwrap();
            MetaNode::init(&mut page_guard.data);
        }

        // Create initial root (leaf) page
        let (root_page_id, root_page) = bpm.new_page()?;
        {
            let mut page_guard = root_page.write().unwrap();
            LeafNode::init(&mut page_guard.data);
        }

        // Set root in meta page
        {
            let mut meta_guard = meta_page.write().unwrap();
            MetaNode::set_root_page_id(&mut meta_guard.data, Some(root_page_id));
        }
    ...
    /// Get meta page ID (this is what gets stored in pg_index)
    pub fn meta_page_id(&self) -> Option<u32> {
        self.meta_page_id
    }

    /// Set meta page ID (used when loading existing index)
    pub fn set_meta_page_id(&mut self, page_id: u32) {
        self.meta_page_id = Some(page_id);
    }

    /// Read root page ID from meta page
    fn get_root_page_id(&self) -> Result<Option<u32>> {
        let meta_page_id = match self.meta_page_id {
            Some(id) => id,
            None => return Ok(None),
        };

        let mut bpm = self.bpm.lock().unwrap();
        let page = bpm.fetch_page(meta_page_id)?;
        let page_guard = page.read().unwrap();
        let root_id = MetaNode::get_root_page_id(&page_guard.data);
        drop(page_guard);
        drop(bpm);
        self.bpm.lock().unwrap().unpin_page(meta_page_id, false)?;
        Ok(root_id)
    }

    /// Update root page ID in meta page
    fn set_root_page_id(&self, new_root: u32) -> Result<()> {
        let meta_page_id = self
            .meta_page_id
            .ok_or_else(|| anyhow::anyhow!("no meta page"))?;

        let mut bpm = self.bpm.lock().unwrap();
        let page = bpm.fetch_page_mut(meta_page_id)?;
        {
            let mut page_guard = page.write().unwrap();
            MetaNode::set_root_page_id(&mut page_guard.data, Some(new_root));
        }
        drop(bpm);
        self.bpm.lock().unwrap().unpin_page(meta_page_id, true)?;
        Ok(())
    }
    ...
    pub fn search(&self, key: &IndexKey) -> Result<Option<Rid>> {
        let root_id = match self.get_root_page_id()? {
            Some(id) => id,
            None => return Ok(None),
        };
    ...
    pub fn insert(&mut self, key: &IndexKey, rid: Rid) -> Result<()> {
        ...
        if self.meta_page_id.is_none() {
            self.create_empty()?;
        }

        let root_id = self.get_root_page_id()?.unwrap();
        let leaf_page_id = self.find_leaf(root_id, key)?;
    ...
    fn insert_into_internal(
        ...
        if InternalNode::need_split(&page_guard.data, key) {
            ...
            // Split the node
            let (split_key, _) =
                InternalNode::split(&mut page_guard.data, &mut new_page_guard.data);
...
mod tests {
    ...
    fn test_btree_create_empty() {
        ...
        assert!(btree.meta_page_id().is_some());
    }
    ...
    fn test_btree_insert_and_search() {
        ...
        assert_eq!(
            btree.search(&IndexKey::single(Value::Int(15))).unwrap(),
            None
        );
    }
...
mod duplicate_tests {
    ...
    fn test_duplicate_keys() {
        ...
        let key = IndexKey::single(Value::Int(100));
        let results = btree
            .range_scan(Some(&key), Some(&IndexKey::single(Value::Int(101))))
            .unwrap();
        assert_eq!(
            results.len(),
            10,
            "Should find all 10 duplicate keys, found {}",
            results.len()
        );
    }
    ...
    fn test_duplicate_keys_with_split() {
        ...
        let key = IndexKey::single(Value::Int(50));
        let results = btree
            .range_scan(Some(&key), Some(&IndexKey::single(Value::Int(51))))
            .unwrap();
        assert_eq!(
            results.len(),
            200,
            "Should find all 200 duplicate keys, found {}",
            results.len()
        );
    }
    ...
    fn test_duplicate_keys_large() {
        ...
        assert_eq!(
            results.len(),
            1000,
            "Should find all 1000 duplicate keys, found {}",
            results.len()
        );
    }
...
mod edge_case_tests {
    ...
    fn test_varchar_key_ordering() {
        ...
        assert_eq!(
            results[0].0,
            IndexKey::single(Value::Varchar("alice".to_string()))
        );
        assert_eq!(
            results[1].0,
            IndexKey::single(Value::Varchar("bob".to_string()))
        );
        assert_eq!(
            results[2].0,
            IndexKey::single(Value::Varchar("charlie".to_string()))
        );
        assert_eq!(
            results[3].0,
            IndexKey::single(Value::Varchar("david".to_string()))
        );
    }
    ...
    fn test_single_entry_operations() {
        ...
        assert_eq!(
            btree.search(&IndexKey::single(Value::Int(0))).unwrap(),
            None
        );
