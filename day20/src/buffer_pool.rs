...
impl Frame {
    fn new() -> Self {
        Frame {
            page: Arc::new(RwLock::new(HeapPage::new(0))),
            page_id: None,
            pin_count: 0,
            is_dirty: false,
        }
...
pub struct BufferPoolManager<R: Replacer = LruReplacer> {
    ...
    // Dirty Page Table: page_id -> rec_lsn (first LSN that dirtied the page)
    dirty_page_table: HashMap<u32, Lsn>,
}

impl BufferPoolManager<LruReplacer> {
    pub fn new(disk_manager: DiskManager, wal_manager: Arc<WalManager>) -> Self {
        Self::with_replacer(disk_manager, LruReplacer::new(BUFFER_POOL_SIZE), wal_manager)
    }
}

impl<R: Replacer> BufferPoolManager<R> {
    pub fn with_replacer(disk_manager: DiskManager, replacer: R, wal_manager: Arc<WalManager>) -> Self {
    ...
    fn fetch_page_internal(&mut self, page_id: u32, is_dirty: bool) -> Result<Arc<RwLock<HeapPage>>> {
        ...
        let frame = &mut self.frames[frame_id];
        *frame.page.write().unwrap() = HeapPage::from_bytes(&data);
    ...
    pub fn fetch_page(&mut self, page_id: u32) -> Result<Arc<RwLock<HeapPage>>> {
        self.fetch_page_internal(page_id, false)
    }

    pub fn fetch_page_mut(&mut self, page_id: u32) -> Result<Arc<RwLock<HeapPage>>> {
        self.fetch_page_internal(page_id, true)
    }

    pub fn new_page(&mut self) -> Result<(u32, Arc<RwLock<HeapPage>>)> {
        let frame_id = self.find_or_create_frame()?;
        let page_id = self.disk_manager.allocate_page()?;
        println!("[BufferPool] Allocating new page {page_id}");

        let frame = &mut self.frames[frame_id];
        *frame.page.write().unwrap() = HeapPage::new(page_id);
        frame.page_id = Some(page_id);
        frame.pin_count = 1;
        frame.is_dirty = true;
    ...
    pub fn flush_all(&mut self) -> Result<()> {
        // Flush all WAL first (Write-Ahead)
    ...
    // Get snapshot of DPT for checkpoint
    pub fn get_dpt_snapshot(&self) -> HashMap<u32, Lsn> {
        self.dirty_page_table.clone()
    }

    // Restore DPT from checkpoint (for recovery)
