...
impl Instance {
    pub fn new(init: bool) -> Result<Self> {
        ...
        let _stats = RecoveryManager::recover(DATA_DIR, WAL_DIR, &bpm, &wal_manager, &txn_manager)?;
    ...
    fn handle_client(
        ...
        loop {
            match conn.read_message()? {
                Some(FrontendMessage::Query(sql)) => {
                    ...
                    match Self::execute_sql_internal(
                        &sql,
                        &catalog,
                        &bpm,
                        &lock_manager,
        ...
        if txn.is_active() {
            ...
            let _ =
                ExecutionEngine::perform_rollback(&bpm, &wal_manager, txn_id, undo_log, last_lsn);
            // Write Abort record
    ...
    fn execute_sql_internal(
        ...
        match &stmt {
            ...
            Statement::Rollback => {
                ...
                let new_last_lsn = ExecutionEngine::perform_rollback(
                    bpm,
                    wal_manager,
                    txn_id,
                    undo_log,
    ...
    fn perform_checkpoint(
        ...
        println!(
            "[Checkpoint] Checkpoint completed at LSN {}",
            checkpoint_lsn
        );

        // Delete old WAL segments that are no longer needed
        if let Some(min_rec_lsn) = bpm
            .lock()
            .unwrap()
            .get_dpt_snapshot()
            .values()
