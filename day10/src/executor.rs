...
impl Executor for SeqScanExecutor<'_> {
    ...
    fn next(&mut self) -> Result<Option<Tuple>> {
        ...
        loop {
            ...
            // Acquire shared lock BEFORE checking if tuple exists
            if let Some(ref mut txn) = self.txn {
                if txn.is_active() {
                    if let Some(lock_manager) = self.lock_manager {
                        lock_manager
                            .lock(txn.id, rid, LockMode::Shared)
            ...
            // Now fetch page and check if tuple exists
            let page_arc = self.bpm.lock().unwrap().fetch_page(self.current_page_id)?;
            let result = {
                let page_guard = page_arc.read().unwrap();
...
impl Executor for InsertExecutor<'_> {
    ...
    fn next(&mut self) -> Result<Option<Tuple>> {
        ...
        // Acquire exclusive lock and record undo log if in transaction
        if let Some(ref mut txn) = self.txn {
            if txn.is_active() {
                if let Some(lock_manager) = self.lock_manager {
                    lock_manager
...
impl Executor for UpdateExecutor<'_> {
    ...
    fn next(&mut self) -> Result<Option<Tuple>> {
        ...
        for (old_rid, old_data, new_values) in updates {
            ...
            // Acquire lock on new RID and record undo log if in transaction
...
impl<'a> ExecutionEngine<'a> {
    pub fn new(
        ...
        let executor = Self::build_executor(bpm, catalog, stmt, txn, lock_manager)?;
    ...
    fn build_executor(
        ...
        match stmt {
            AnalyzedStatement::Select(s) => Ok(Self::build_select_executor(bpm, catalog, s, txn, lock_manager)),
            AnalyzedStatement::Insert(s) => {
                Ok(Box::new(InsertExecutor::new(bpm, s, txn, lock_manager)?))
            }
            AnalyzedStatement::CreateTable(_) => {
                anyhow::bail!("CREATE TABLE not implemented in executor")
            }
            AnalyzedStatement::Delete(s) => {
                Ok(Self::build_delete_executor(bpm, catalog, s, txn, lock_manager))
            }
            AnalyzedStatement::Update(s) => {
                Ok(Self::build_update_executor(bpm, catalog, s, txn, lock_manager))
            }
        }
    }
    ...
    fn build_select_executor(
        ...
        // SeqScan
        let scan: Box<dyn Executor> = Box::new(SeqScanExecutor::new(bpm, catalog, table_id, txn, lock_manager));
    ...
    fn build_delete_executor(
        ...
        // SeqScan (no lock - lock acquired in DeleteExecutor)
        let scan: Box<dyn Executor> =
            Box::new(SeqScanExecutor::new(Arc::clone(&bpm), catalog, table_id, None, None));
        ...
        Box::new(DeleteExecutor::new(bpm, filtered, txn, lock_manager))
    }
    ...
    fn build_update_executor(
        ...
        // SeqScan (no lock - lock acquired in UpdateExecutor)
        let scan: Box<dyn Executor> =
            Box::new(SeqScanExecutor::new(Arc::clone(&bpm), catalog, table_id, None, None));
