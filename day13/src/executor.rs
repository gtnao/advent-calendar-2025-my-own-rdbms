...
impl Executor for DeleteExecutor<'_> {
    ...
    fn next(&mut self) -> Result<Option<Tuple>> {
        ...
        // Delete all targets and record WAL + undo log
...
impl<'a> ExecutionEngine<'a> {
    pub fn new(
        ...
        let executor = Self::build_executor(bpm, catalog, stmt, txn, lock_manager, wal_manager)?;
    ...
    pub fn perform_rollback(
        ...
        for entry in undo_log.into_iter().rev() {
            match entry {
                ...
                UndoLogEntry::Delete { prev_lsn, rid, data, .. } => {
                    ...
                    let clr_lsn = wal_manager.append(
                        ...
                        WalRecordType::CLR {
                            undo_next_lsn: prev_lsn,
                            redo: CLRRedo::UndoDelete { rid, data },
                        },
                    );
                    page_guard.page_lsn = clr_lsn;
                    last_lsn = clr_lsn;
    ...
    fn build_executor(
        ...
        match stmt {
            ...
            AnalyzedStatement::Delete(s) => {
                Ok(Self::build_delete_executor(bpm, catalog, s, txn, lock_manager, wal_manager))
            }
            AnalyzedStatement::Update(s) => {
                Ok(Self::build_update_executor(bpm, catalog, s, txn, lock_manager, wal_manager))
            }
        }
    }
